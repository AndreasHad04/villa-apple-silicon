"""Generate the PR 1812 comment from results/mps_model_coverage.json."""
import json, pathlib
R = pathlib.Path(__file__).resolve().parent.parent
d = json.load(open(R / "results" / "mps_model_coverage.json"))
c = d["trained_weight_control"]
rows = {r["model_type"]: r for r in d["rows"]}

def cell(r, k):
    v = r.get(k)
    return "not finite" if v is None else f"{v:g}"

tbl = ["| `MODEL_TYPE` | frames | MPS fp32 vs CPU, max abs diff | MPS autocast vs CPU, max abs diff |",
       "|---|---:|---:|---:|"]
for k in ("timesformer", "resnet3d-50", "resnet3d-152", "resnet3d-152-3d-decoder"):
    r = rows[k]
    tbl.append(f"| `{k}`{' (the default)' if k == 'timesformer' else ''} | {r['num_frames']} | "
               f"{cell(r,'mps_fp32_max_abs_diff')} | {cell(r,'mps_amp_max_abs_diff')} |")

txt = f"""Two follow-ups: a gap in my own validation that I have now closed, and a cross reference to work on the sibling inference path that a reviewer should see next to this.

**The gap. Everything measured in this PR used one model type, and it is not the default.** `optimized_inference` dispatches four (`entrypoint.py:717`): `timesformer`, `resnet3d-50`, `resnet3d-152` and `resnet3d-152-3d-decoder`, and `InferenceInputs.model_type` defaults to `timesformer`. Every number in this PR came from `resnet3d-152-3d-decoder`. A device patch can fail in exactly two ways, a missing MPS kernel or a numeric divergence, and neither depends on trained weights, so I ran each architecture with random weights on both devices with the same input. torch {d['torch']}, MPS available {str(d['mps_available']).lower()}, 64 px input, one tile.
...
"""
out = R / "results" / "PR1812_COMMENT_coverage.md"
out.write_text(txt)
bad = sum(txt.count(ch) for ch in (chr(0x2014), chr(0x2013)))
low = txt.lower()
ai = sum(low.count(w) for w in ("claude", "chatgpt", "copilot", "generated with", "co-authored"))
print(f"wrote {out}  {len(txt)} chars  dashes={bad}  ai={ai}")
assert bad == 0 and ai == 0
