"""ARM Z, stage 2: GENERATED from ops/ys_infer.py by replacing "ys" with "zs"; do not edit by hand.
...
def main(argv):
    ...
    segs = argv or SEGS
    log = ROOT / "results" / "zs" / "infer_runs.jsonl"
    villa_sha = os.popen(f"git -C {ROOT / 'villa'} rev-parse HEAD").read().strip()
    for seg in segs:
        sd = ROOT / "data" / "zs" / seg; prep = json.load(open(sd / "prep.json"))
        ...
        for w in wins:
            ...
            for ck in XI.checkpoints(csel):
                for d in ("forward", "reverse"):
                    out = ROOT / "results" / "zs" / "pred" / seg / w / f"{ck.parent.name}_{ck.stem}_{d}.tif"
