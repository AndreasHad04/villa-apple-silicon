...
**It is not geometry.** Without labels or a model, the eligible render's sheet sits within half a layer of the 2.403 um one in depth. It differs in blur: its adjacent layers correlate {corr_n:.3f} on average against {corr_p:.3f} for the 2.403 um input the model mostly trained on (and {corr_800:.3f} on a PHerc0800 8.64 um render, the other eligible configuration).
...
**Depth is the lever (found after the primary failed, so tested again on fresh data).** Sharpening each pixel's 21-sample depth profile with a fixed gain, plus the same intensity map, raised the PHerc0841 median by {sg(ndh)} and improved {N14['pairs_improved']} of {N14['pairs']} checkpoint and segment pairs across all 14 released checkpoints. Fresh tests, transforms fitted on PHerc0841 only:
{fresh_lines()}"""
...
issue = f"""ink_9um loses AUC on the eligible 1.2 m scans mainly through depth blur; {head}
...
"""
discord = f"""ink_9um on the eligible 1.2 m scans: the AUC it loses against 2.4 um inputs is mostly depth blur, not geometry and not in-plane resolution. On PHerc0841 a fixed, label-free depth filter raised the median by {sg(ndh)} ({N14['pairs_improved']} of {N14['pairs']} checkpoint pairs); on held-out scrolls: {'; '.join(f"{lab.split(' (')[0]} {sg(m)}" for lab, _, m, _ in fresh)}. The pre-registered primary endpoint (an in-plane fix) failed. Tool and numbers: {SEC}
"""
yz = Y["Y2"]; zz = Z["Z1"]
