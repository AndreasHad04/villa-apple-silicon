"""ARM H2: does bf16 work on MPS for hecate, and what does it cost?

hecate.py:322 refuses bf16 on any non-CUDA device. MPS bf16 is measured working
(ops/feasibility.py). This runs the SAME pipeline twice, changing only that one
line, and reports min/median/max per villa AGENTS.md 1.4.
"""
...
var  = load_mod("hec_bf16", f"{HERE}/hecate/hecate_bf16mps.py")

res = {"volume_shape": list(vol.shape), "reps": REPS}

# negative control FIRST: the unmodified module must REFUSE bf16 on mps
try:
    m = base.load_model(CKPT, device="mps")
    o = np.zeros((vol.shape[1], vol.shape[2]), dtype=np.uint8)
    base.predict(m, vol, o, precision="bf16")
    res["upstream_refuses_bf16_on_mps"] = False
except ValueError as e:
    res["upstream_refuses_bf16_on_mps"] = True
    res["upstream_refusal_message"] = str(e)
print("control: upstream refuses bf16 on mps =", res["upstream_refuses_bf16_on_mps"], flush=True)

t32, o32 = run(base, "fp32", vol, REPS)
tbf, obf = run(var,  "bf16", vol, REPS)

def stats(t): return {"min": min(t), "median": float(np.median(t)), "max": max(t), "all": t}
d = np.abs(o32.astype(np.int16) - obf.astype(np.int16))
res.update({
    "fp32_mps": stats(t32), "bf16_mps": stats(tbf),
...
json.dump(res, open(f"{HERE}/results/hecate_bf16.json", "w"), indent=1)
